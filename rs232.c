...
/*
  0 ..  1   ttyAMA? - hardware serial
  2 ..  3   ttyACM?
  4 ..  9   ttyUSB? - USB->RS232 converters
 10 .. 25   ttyS?
 26 .. 27   rfcomm?
 28 .. 29   ircomm?
 30 .. 37   cuau? / cuaU?
*/
char comports[38][16]={"/dev/ttyAMA0","/dev/ttyAMA1","/dev/ttyACM0","/dev/ttyACM1","/dev/ttyUSB0",
...
*/
  ...
  error = tcgetattr(Cport, old_port_settings + comport_number);
  if(error==-1)
  {
    close(Cport);
...
void RS232_CloseComport(int comport_number)
{
  ...
  tcsetattr(Cport, TCSANOW, old_port_settings + comport_number);
  close(Cport);
...
/*
char comports[24][10]={"\\\\.\\COM1",  "\\\\.\\COM2",  "\\\\.\\COM3",  "\\\\.\\COM4",
                       "\\\\.\\COM5",  "\\\\.\\COM6",  "\\\\.\\COM7",  "\\\\.\\COM8",
...
void RS232_flushRX(int comport_number)
{
  PurgeComm(Cport, PURGE_RXCLEAR | PURGE_RXABORT);
}


void RS232_flushTX(int comport_number)
{
  PurgeComm(Cport, PURGE_TXCLEAR | PURGE_TXABORT);
}
